...
// Convertir resultados a arrays para JavaScript
$ageGroupsData = [];
if ($ageGroups && $ageGroups instanceof mysqli_result) {
    while($row = $ageGroups->fetch_assoc()) {
        $ageGroupsData[] = $row;
    }
} else {
    error_log("Error al obtener grupos de edad: " . $mysqli->error);
}

$genderData = [];
if ($genderStats && $genderStats instanceof mysqli_result) {
    while($row = $genderStats->fetch_assoc()) {
        $genderData[] = $row;
    }
} else {
    error_log("Error al obtener estadísticas de género: " . $mysqli->error);
}

$altitudeData = [];
if ($altitudeRanges && $altitudeRanges instanceof mysqli_result) {
    while($row = $altitudeRanges->fetch_assoc()) {
        $altitudeData[] = $row;
    }
} else {
    error_log("Error al obtener rangos de altitud: " . $mysqli->error);
}

$departmentData = [];
if ($departmentStats && $departmentStats instanceof mysqli_result) {
    while($row = $departmentStats->fetch_assoc()) {
        $departmentData[] = $row;
    }
} else {
    error_log("Error al obtener estadísticas por departamento: " . $mysqli->error);
}

$provincesData = [];

if ($topProvinces && $topProvinces instanceof mysqli_result) {
    while ($row = $topProvinces->fetch_assoc()) {
        $provincesData[] = $row;
    }
} else {
    // Muestra el error de MySQL si hubo problema con la consulta
    echo "Error al obtener datos de provincias: " . $mysqli->error;
}
