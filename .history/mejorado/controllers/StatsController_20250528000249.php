...
public function getDashboardSummary() {
    ...
    if (!$genderResult) {
        throw new Exception("Error al obtener estadísticas de género: " . $this->mysqli->error);
    }
    $summary['gender'] = [];
    ...
    if (!$anemiaResult) {
        throw new Exception("Error al obtener estadísticas de anemia: " . $this->mysqli->error);
    }
    $summary['anemia'] = [];
    $count = 0;
    while ($row = $anemiaResult->fetch_assoc()) {
        if ($count >= 5) break;
        $summary['anemia'][] = $row;
        $count++;
    }
