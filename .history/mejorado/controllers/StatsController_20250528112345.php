...
function obtenerCasosAnemiaPorSexo(PDO $pdo): array {
    $sql = "
        SELECT sexo, COUNT(*) AS casos
        FROM datos_salud
        WHERE dx_anemia IS NOT NULL AND dx_anemia <> ''
        GROUP BY sexo
    ";
    ...
    // Inicializar con 0 por si no hay datos
    $casos = ['Hombre' => 0, 'Mujer' => 0];

    foreach ($result as $row) {
        $sexo = ucfirst(strtolower($row['sexo']));
        if (isset($casos[$sexo])) {
            $casos[$sexo] = (int)$row['casos'];
        }
    }
