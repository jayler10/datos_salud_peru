...
class StatsController {
 ...
 public function getGeneralSummary() {
    ...
    /**
     * Distribución de casos por mes (últimos 12 meses)
     */
    public function getCasesByMonth() {
        $query = "
            SELECT 
                DATE_FORMAT(fecha_nacimiento, '%Y-%m') as mes,
                COUNT(*) as casos
            FROM datos_salud 
            WHERE fecha_nacimiento >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
    ...
    public function getAlerts() {
        $query = "
            SELECT 
                'Nivel crítico de hemoglobina' as tipo_alerta,
                COUNT(*) as cantidad
            FROM datos_salud 
            WHERE hemoglobina < 7.0
            UNION ALL
            SELECT 
                'Casos de anemia severa' as tipo_alerta,
                COUNT(*) as cantidad
            FROM datos_salud 
            WHERE dx_anemia LIKE '%severa%'
            UNION ALL
            SELECT 
                'Registros sin diagnóstico' as tipo_alerta,
                COUNT(*) as cantidad
            FROM datos_salud 
            WHERE dx_anemia IS NULL OR dx_anemia = ''
        ";
        return $this->mysqli->query($query);
    }
    
    /**
     * Estadísticas rápidas por género
     */
    public function getQuickGenderStats() {
        $query = "
            SELECT 
                sexo,
                COUNT(*) as total,
                ROUND(AVG(hemoglobina), 2) as hemoglobina_promedio,
                COUNT(CASE WHEN dx_anemia IS NOT NULL AND dx_anemia != '' THEN 1 END) as con_anemia
            FROM datos_salud 
            GROUP BY sexo
        ";
        return $this->mysqli->query($query);
