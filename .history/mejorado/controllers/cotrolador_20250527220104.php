...
class StatsController {
    ...
    /**
 * Estadísticas de anemia por sexo
 */
    ...
    $query = "
        SELECT 
            ...
            ROUND(COUNT(*) * 100.0 / (
                SELECT COUNT(*) 
                FROM datos_salud 
                WHERE sexo = ds.sexo AND dx_anemia != '' AND dx_anemia IS NOT NULL
            ), 2) as porcentaje_por_sexo,
            ROUND(COUNT(*) * 100.0 / (
                SELECT COUNT(*) 
                FROM datos_salud 
        ...
        ORDER BY sexo, 
            CASE 
                WHEN dx_anemia LIKE '%Sin%' OR dx_anemia LIKE '%Normal%' THEN 1
                WHEN dx_anemia LIKE '%Leve%' THEN 2
                WHEN dx_anemia LIKE '%Moderada%' THEN 3
                WHEN dx_anemia LIKE '%Severa%' OR dx_anemia LIKE '%Grave%' THEN 4
                ELSE 5
            END
    ";
    return $this->mysqli->query($query);
