...
public function getAnemiaByAgeRanges() {
    ...
    SELECT 
        ...
        dx_anemia,
        COUNT(*) AS cantidad,
        ROUND(COUNT(*) * 100 / (SELECT COUNT(*) FROM datos_salud WHERE fecha_nacimiento IS NOT NULL), 2) AS porcentaje
    FROM (
    ...
    if (!$result) {
        die("Error en consulta getAnemiaByAgeRanges: " . $this->mysqli->error);
    }
    return $result;
}
